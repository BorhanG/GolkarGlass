...
document.addEventListener("DOMContentLoaded", () => {
  const orders = [
    {
      ...
      products: [
        {
          name: "شیشه دوجداره ۴ میل",
          qty: 2,
          price: "۲۵۰,۰۰۰ تومان",
          image: "/Product page/placeholder1.jpg",
        },
        {
          name: "قاب طلایی طرح جدید",
          qty: 1,
          price: "۲۰۰,۰۰۰ تومان",
          image: "/Product page/placeholder2.jpg",
        },
      ],
    },
    {
      ...
      products: [
        {
          name: "آینه کلاسیک فریم‌دار",
          qty: 1,
          price: "۲۸۰,۰۰۰ تومان",
          image: "/Product page/placeholder3.jpg",
        },
      ],
    },
    {
  ...
  document.querySelectorAll(".view-btn").forEach((btn, i) => {
    btn.addEventListener("click", () => {
      ...
      if (!order) {
        console.warn("No order found for this button");
        return;
      }
      openModal(order);
    });
...
function renderWishlist() {
  ...
  if (wishlist.length === 0) {
    wishlistContainer.innerHTML =
      '<div class="empty-message">لیست علاقه‌مندی‌های شما خالی است</div>';
    return;
  }
...
function attachWishlistEventListeners() {
  ...
  wishlistContainer.addEventListener("click", function (e) {
    ...
    const itemName = wishlistItem.querySelector(
      ".wishlist-item-title"
    ).textContent;
...
document.addEventListener("DOMContentLoaded", function () {
        ...
        switch (action) {
          ...
          case "سفارشات من":
            window.location.href =
              "/My orders page/My orders page/my-orders-advanced.html";
            break;
          case "پروفایل":
  ...
  cartItems.forEach((item) => {
    ...
    if (minusBtn && plusBtn && qtyNumber) {
      minusBtn.addEventListener("click", function () {
        ...
        if (currentQty > 1) {
          ...
          saveCartToStorage();
          showNotification(
            `تعداد ${item.querySelector(".cart-item-title").textContent} به ${
              currentQty - 1
            } کاهش یافت`,
            "info"
          );
    ...
    if (removeBtn) {
      removeBtn.addEventListener("click", function () {
        ...
        if (
          confirm(`آیا مطمئن هستید که می‌خواهید ${productName} را حذف کنید؟`)
        ) {
  ...
  wishlistItems.forEach((item) => {
    ...
    if (removeWishlistBtn) {
      removeWishlistBtn.addEventListener("click", function () {
        ...
        if (
          confirm(
            `آیا مطمئن هستید که می‌خواهید ${productName} را از علاقه‌مندی‌ها حذف کنید؟`
          )
        ) {
          item.style.animation = "slideOut 0.3s ease forwards";
          setTimeout(() => {
            item.remove();
            showNotification(
              `${productName} از علاقه‌مندی‌ها حذف شد`,
              "success"
            );
          }, 300);
        }
    ...
    btn.addEventListener("click", function () {
      ...
      switch (action) {
        ...
        case "سفارش مجدد":
          ...
          if (
            confirm("آیا مطمئن هستید که می‌خواهید این سفارش را تکرار کنید؟")
          ) {
  ...
  if (checkoutBtn) {
    checkoutBtn.addEventListener("click", function () {
      ...
      if (result.total > 0) {
        ...
        showNotification(
          `در حال انتقال به صفحه پرداخت برای مبلغ ${result.total.toLocaleString()} تومان...`,
          "success"
        );
  ...
  function addItemToCart(productName, productPrice, productImage) {
    ...
    minusBtn.addEventListener("click", function () {
      ...
      if (currentQty > 1) {
        ...
        const result = updateCartTotal();
        showNotification(
          `تعداد ${productName} به ${currentQty - 1} کاهش یافت`,
          "info"
        );
      } else {
        showNotification("حداقل تعداد محصول ۱ است", "error");
    ...
    plusBtn.addEventListener("click", function () {
      ...
      const result = updateCartTotal();
      showNotification(
        `تعداد ${productName} به ${currentQty + 1} افزایش یافت`,
        "info"
      );
    });
  ...
  function showCartSummary() {
    ...
    if (result.totalItems > 0) {
      showNotification(
        `سبد خرید شما شامل ${result.totalItems} محصول به ارزش ${formatPrice(
          result.total
        )} است`,
        "info"
      );
