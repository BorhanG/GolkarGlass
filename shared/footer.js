...
`;
  ...
  function saveTheme(theme){
    try {
      ...
      // keep settingsInfo.theme in sync if present
      var settings = {};
  ...
  function applyTheme(theme){
    try {
      ...
      // Optional class for legacy selectors
  ...
  function handleThemeSelectIfPresent(){
    try {
      ...
      // Initialize select value from saved theme but do NOT auto-apply on change.
      // Theme should only be applied when Settings page explicitly saves.
      var current = getSavedTheme();
      if (themeSelect.value !== current) themeSelect.value = current;
    } catch(e) {}
  }

  // React to theme changes from other tabs/pages or code
  ...
  function appendFooter(){
    try{
      if (document.querySelector('footer.footer')) return; // avoid duplicates
